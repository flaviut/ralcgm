...
*/
{
...
#endif
    if (clip != NULL) {   /*   Clipping requested */
    ...
    if (!overlap && (pset->eofl != NULL)) {   /* Polygon set must not have overlapping polygons */
    ...
    if (ps == NULL) {   /* No section list */

        /* Return the input polygon set */

        clipped = pset;
    } else {  /* We have a section list */
...
#endif
    if (pset->eofl == NULL) {   /* No edge flags (not polygon set) */
        *end = *(pt + pset->n - 1);      /* Define closing edge */
        *start = *pt;
        if (clip == NULL) { /* Not clipping, draw as polyline and close */
            linefunc(pset->n, pt);
            linefunc(2L, close);
#ifdef DEBUG
            DMESS "POLedge: draw the polygon without clipping.\n");
#endif
        } else {  /* Clipping, draw and clip as polyline and close clipped */
        ...
        *start = *pt;                   /* Define start of polygon */
        for (i = 0, nvis = 1; i < pset->n - 1; i++, pt++, fl++) {    /* Loop over vertices and edges */
            switch (*fl) {
                case VIS:
...
Long POLselfx(Long np, Point *pt, Long limit) {
    ...
    for (i = 0; i < np - 1; i++) {
        ...
        for (j = i + 1, eo = end2; j < np; j++, eo++) {  /* determine whether the two edges intersect */
            if ((bbox.left <= eo->x) && (eo->x <= bbox.right) &&
                (bbox.left <= (eo + 1)->x) && ((eo + 1)->x <= bbox.right)) {
                if ((bbox.bot <= eo->y) && (eo->y <= bbox.top) &&
                    (bbox.bot <= (eo + 1)->y) && ((eo + 1)->y <= bbox.top)) {   /* Bounding boxes do intersect */
                    if (GRAcross(end1, end2, eo, eo + 1, &colline, &sx)) {
                        if (!colline) { /* Count crossing, if not collinear */
                            count++;
...

{
    ...
    if ((end1->x < test->x) != (end2->x < test->x)) {  /* Edge goes from one side of testpoint to other */
#ifdef DEBUG_PTS
                                                                                                                                DMESS "goes_over: end1 (%f,%f), end2 (%f,%f), test (%f,%f)\n",
             end1->x,end1->y,end2->x,end2->y,test->x,test->y);
#endif
        if ((end1->y > test->y) && (end2->y > test->y)) {   /* Both ends above test point */
            over = TRUE;
#ifdef DEBUG_PTS
            DMESS "both ends over\n");
#endif
        } else {
            if ((end1->y > test->y) || (end2->y > test->y)) {  /* One end above test point, other below */
                if (end1->x < end2->x) { /* Rightward Edge - (Over if passes test pt clockwise) */
                    over = CLOCK(end1, end2, test);
#ifdef DEBUG_PTS
                    DMESS "Rightward Edge, over %d\n",over);
#endif
                } else { /* Leftward Edge - (Over if passes t.pt anti-clockwise */
...
Point *POLaddcnrs(Enum side1, Enum side2, Logical clockwise,
    ...
    start = freept;
    if (clockwise) {                        /* Add in Clockwise Order */
        switch (side1) {                     /* starting and ending appropriately */
            case CLIPRIGHT:
    ...
    } else {                         /* Add in Anticlockwise Order */
        switch (side1) {                      /* starting and ending approprately */
            case CLIPLEFT:
                freept->x = clip->left;      /* Add Lower-Left Corner */
                freept->y = clip->bot;
                freept++;
...
static void xing_count(Polygonset *pset, Rect *cl,
    ...
    *nsect = 0;
    ...
    for (i = 0, end1 = pset->pts, fl = pset->eofl; i < pset->n; i++, end1++, fl++) {
        ...
        if ((cl1x == 0) && (cl1y == 0)) {  /* 1nd End Inside */
            if ((cl2x != 0) || (cl2y != 0)) {  /* Outgoing Edge */
#ifdef DEBUG_CROSS
        ...
        } else {  /* 1st End Outside */
            if ((cl2x == 0) && (cl2y == 0)) {  /* Incoming edge */
#ifdef DEBUG_CROSS
                ...
                if (((cl1x != cl2x) || (cl1x == 0)) &&
                    ...
                    ((RCLOCK(end1, end2, cl, left, top) ||
                     ...
                     (!RCLOCK(end1, end2, cl, left, top) ||
                      !RCLOCK(end1, end2, cl, right, top) ||
                      !RCLOCK(end1, end2, cl, left, bot) ||
                      !RCLOCK(end1, end2, cl, right, bot)))) {  /* Edge crosses rectangle */
#ifdef DEBUG_CROSS
      ...
      endsection, *nsect);
        ...
        if (endsection) {  /* End of section: count it */
            (*nsect)++;
            endsection = FALSE;
        }
...
static void xing_edge(Polygonset *pset, Rect *cl,
    ...
    for (index = 0, end1 = pset->pts, fl = pset->eofl; index < pset->n;
        ...
        if ((cl1x == 0) && (cl1y == 0)) {   /* 1st end Inside */
            if ((cl2x != 0) || (cl2y != 0)) { /* Outgoing edge */
   ...
   Crossings already done are marked by setting 'out+index' to -1. */
{
    ...
    *(out + xc->cross1) = -1;   /* Mark 1st crossing as done */
    ...
    if (xc->sectype == CLIPINDUCED) { /* Entering Clipping Rectangle */
    ...
    } else {   /* Leaving Clipping Rectangle */
...
*/
{
    ...
    for (iplace = 0; iplace < ncross - 1; iplace++) {  /* NB: 'iplace' crossings have been sorted */
        mplace = iplace;
        icr = ord + iplace;
        icross = *icr; /* First unsorted crossing */
        mcr = icr;
        em = cls + *mcr;
        rm = xy + *mcr;
        for (jplace = iplace + 1, jcr = icr + 1;
             jplace < ncross;
             jplace++, jcr++) { /* Loop over the other unsorted crossings */
            ej = cls + *jcr;
            rj = xy + *jcr;
            if (CROSSORD(*ej, *rj, *em, *rm)) { /* Earlier crossing found: make it the earliest */
                mplace = jplace;
                mcr = jcr;
                em = cls + *mcr;
                rm = xy + *mcr;
        ...
        if (mplace != iplace) {  /* Earliest crossing not 'iplace': swap them */
            mcross = *mcr;
            /* Swap in ordering */
            *mcr = icross;
            *icr = mcross;
...
*/
    ...
    if (ncross > 0) {  /* There is at least one crossing of the clipping rectangle
      ...
      *(cross.ord),*(cross.xy + *(cross.ord)));
        ...
        ntotop = 0;
        xy = cross.xy;
        if (ntop == 0) {  /* No top crossings: select mid point of top */
            top.x = (0.5) * (cl->left + cl->right);
        } else {  /* Find a point not on a polygon edge */
            if (VEQUAL(cl->right, *(xy + *(cross.ord)))) {  /* Earliest crossing on upper-right corner (unlikely) */
                for (icross = 0, icr = cross.ord; icross < ntop - 1; icross++, icr++) {
                    if (!VEQUAL(*(xy + *icr), *(xy + *(icr + 1)))) { /* Crossings have a gap between them */
                        top.x = (0.5) * (*(xy + *icr) + *(xy + *(icr + 1)));
                        ntotop = icross + 1;
                        break;
                    }
                }
                if (ntotop == 0) {  /* No gaps between crossings */
                    top.x = (0.5) * (*(xy + *icr) + cl->left);
                    ntotop = ntop;
                }
            } else {   /* Gap between earliest edge and upper-right corner */
#ifdef DEBUG
      ...
      if(earlin) DMESS "POLclip_sl: selected top pt INSIDE poly");
        ...
        /* Determine whether there is a clip induced edge section that
         ...
         one end.
      */
        if (earlin) {        /* First corner of clip rectangle is inside polygon */
      ...
      each clipped section */
        ...
        for (icross = 0, pol = cross.pol, oth = cross.inoth;
             icross < ncross - 1;
             icross++, pol++, oth++) {
            if (!polyset || (*pol == *(pol + 1))) {  /* Crossing in same polygon as the next crossing */
                if (inwards) {  /* Inward crossing */
            ...
            } else {  /* Last crossing of current polygon & new polygon to follow */
                if (inwards) {  /* Inward crossing */
                    *oth = firstcross;
    ...
    {  /* Define the Sections of Clipped Polygons */
        ...
        *firsec = ps->vacant;
        ...
        if (polyset) {   /* Must be polygonset, if any polygons are entirely inside */
            ...
            for (icross = 0; icross < ncross + 1; icross++) {
                ...
                if (hipol > lopol + 1) {  /* Block of consecutive Ommitted Polygons Detected */
#ifdef DEBUG
                   ...
                   clipping rectangle */
                    for (curpol = lopol + 1; curpol < hipol; curpol++) {
                        ...
                        if (!CLIPCODE(in->x, cl->left, cl->right) &&
                            !CLIPCODE(in->y, cl->bot, cl->top)) {  /* Polygon is Inside Rect, add it as section */
...
# ifdef DEBUG_SECT
        ...
        if (ncross > 0) {  /* There are crossings */
...
#endif
                    if (polsta != NULL) {  /* Section includes closing edge of unclipped polyon:
                     split there */
        ...
        DMESS "POLclip_sl: section split at closing edge: sectype %d,",
                  ...
                  DMESS " edge (%d to %d)\n",spl2,spl1);
                    ...
                    if (xcr.sectype != NOSECTION) { /* Crossings form the ends of a section,
                    add the section */
                    ...
                    if (xing_next(cross.in, cross.out, cross.inoth, cross.outoth,
                                  &xcr) == NULL) {   /* End of clipped polygon reached, find next one */
                        firsec++;
                        *firsec = ps->vacant;
               ...
               *(cross.out + xcr.cross2), xcr.cross2);
                    ...
                    /* Clockwise section? */
                    if (side1 == side2) { /* Both ends on same side - need to see if edge section
                                             goes round rectangle */
                        if (round4) {
                            if ((*cross.ord == xcr.cross1) ||
                                *cross.ord == xcr.cross2) { /* This clip induced section goes round
                          all four corners of the clipping rectangle */
                                ps->freept =
                                        POLaddcnrs(side1, side2, clockwise, cl, ps->freept);
                            }
                        }
                    } else { /* Ends are on different sides - need to add corner(s)
            ...
            xing_free(&cross);
        } else {   /* No crossings of the clipping rectangle boundary by
             polygon boundary. */

            /* Determine if upper-right corner is inside polygon set */
            top.x = cl->right;
            top.y = cl->top;
            if (POLin(pset, &top)) { /* Corner is inside, add rectangle as a section */
                ps->vacant->start = ps->freept;
...
*/
{
    ...
    for (i = 0, fromsec = ps->first;
         ...
         i++, fromsec++) {
        if (fromsec->start == NULL) {       /* Gap has been encontered */
            ingap = TRUE;
            shift = TRUE;
        }

        if (*(ps->poly + poly + 1) == fromsec) {        /* Next polygon (or end) has been reached */
            ingap = FALSE;
            poly++;
            if (shift) {
...
void POLcount_sl(Sectlist *ps, Int poly, Int *ns, Long *np) {
    ...
    if (np == NULL) { /* Number of vertices is not requested - simpler algorithm */
        if (ns != NULL) {
            if (ps->contig) *ns = *(ps->poly + poly + 1) - *(ps->poly + poly);
            else { /* non-contiguous section list */
...
#endif
        if (*(ps->poly + poly + 1) < *(ps->poly + poly)) {  /* Bad Polygon List Structure Detected */
#ifdef DEBUG
...
*/
{
    ...
    if (ps == NULL) {  /* No section list supplied - create one */
    ...
    } else {  /* Section list already exists */
        if (ps->msect < nsect) {  /* More sections needed */
            oldps = *ps;
        ...
        if (ps->mpoly < npoly) {   /* More polygons needed */
        ...
        if (ps->mpts < npts) {  /* More points needed */
            oldps = *ps;
            ...
            for (cursec = ps->first; cursec < ps->vacant; cursec++) {  /* Repoint sections that use newpoints */
                if ((cursec->start >= oldps.newpts) &&
                    (cursec->start < oldps.freept)) {  /* Section uses newpoints */
...
void POLinfind_sl(Sectlist *ps, Int *inpoly, Int *outpoly) {
    ...
    for (poly1 = 0; poly1 < ps->npoly - 1; poly1++) {
        for (poly2 = poly1 + 1; poly2 < ps->npoly; poly2++) {
            if (POLinpoly_sl(ps, poly1, poly2)) { /* 1st polygon is inside 2nd */
                *inpoly = poly1;
                *outpoly = poly2;
                found = TRUE;
            } else {  /* 1st polygon not inside 2nd */
                if (POLinpoly_sl(ps, poly2, poly1)) {  /* 2nd polygon is inside 1st */
                    *inpoly = poly2;
                    *outpoly = poly1;
                    found = TRUE;
                }
...
Logical POLinpoly_sl(Sectlist *ps, Int inpoly, Int outpoly) {
    ...
    if (!((test->x < box.left) || (box.right < test->x) ||
          (test->y < box.bot) || (box.top < test->y))) {  /* test point is in bounding box of polygon */
        end1 = a.p;
        for (ip = 0; ip < np; ip++) {
            MOVE_SCURSOR(ps, a);
            end2 = a.p;
...
void POLjoin_sl(Sectlist *ps, Scursor *sc1, Scursor *sc2, Point *join) {
    ...
    for (i = 0; i < sc1->lastsec - sc1->firstsec + 1; i++) {
        ...
        ps->vacant->start = a.p;
        if (a.s == sc1->s) {  /* We're back at the first section */
            ps->vacant->end = sc1->p;
            ps->vacant++;
            break;
        } else {
    ...
    for (i = 0; i < sc2->lastsec - sc2->firstsec + 1; i++) {
        POLnext_sl(ps, &a);  /* Move to start of next section */
        ps->vacant->start = a.p;
        if (a.s == sc2->s) {  /* We're back at the first section */
            ps->vacant->end = sc2->p;
            ps->vacant++;
            break;
        } else {
...
void POLnext_sl(Sectlist *ps, Scursor *sc) {
    ...
    if ((sc->s == sc->lastsec) || ((sc->s + 1)->start == NULL)) { /* End of polygon reached */
        sc->s = sc->firstsec;   /* Back to start of polygon */
    } else { /* Next section in polygon is next in list */
        sc->s++;
    }
...
*/
{
            ...
            for (ip1 = 0; ip1 < np1; ip1++) {
                ...
                for (poly2 = poly1 + 1; poly2 < ps->npoly; poly2++) {
                    ...
                    if (!((box1.right < box2.left) || (box2.right < box1.left) ||
                          (box1.top < box2.bot) || (box2.top < box1.bot))) {  /* Bounding boxes intersect */
    ...
    if (possin && (ps->npoly > 1)) {  /* One polygon may be entirely inside another */
...
Sectlist *POLpoly_sl(Polygonset *pset, Int nsect) {
    ...
    firsec = ps->poly;
    *firsec = ps->vacant;
    if (npoly == 1) {  /* Single polygon: one section for whole */
        ps->vacant->start = pset->pts;            /* 1st Point */
        ps->vacant->end = pset->pts + pset->n;   /* Last Point */
        ps->vacant++;
    } else {  /* Polygon Set: One section per polygon */
        ps->vacant->start = pset->pts;   /* 1st Point */
        curfl = pset->eofl;          /* Initialise Current edge flag */
        for (i = 0; i < pset->n - 1; i++, pset->pts++, curfl++) {
            if (*curfl == CLOSEVIS || *curfl == CLOSEINVIS) {    /* End of polygon: close section */
                ps->vacant++;                  /* Next section */
                firsec++;                      /* Next polygon */
                if (ps->vacant >= ps->first + ps->msect) {  /* Bug: Section List overflow! */
...
Polygonset *POLpts_sl(Sectlist *ps, Polygonset *space) {
    ...
    } else {  /* Supplied space is sufficient, use it */
        pset = *space;
    }
    ...
    if (ps->npoly == 1) { /* Only one polygon */
...

{
    ...
    if ((-1 < ipoly) && (ipoly < ps->npoly) && (ipoly < ps->mpoly)) {  /* Polygon is in polygon list */
        sc->firstsec = sc->s = *(ps->poly + ipoly);
        sc->lastsec = *(ps->poly + ipoly + 1) - 1;
        if (!ps->contig) {  /* Sections not contiguous: find true last section */
    ...
    } else {  /* Polygon not in polygon list */
