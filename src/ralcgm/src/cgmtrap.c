 ...
 */
    ...
    if (clipped == pset) {  /* Clipped polygon(set) same as unclipped, make copy */
#ifdef DEBUG
    ...
    if (clipped != NULL) {  /* Clipping went OK, proceed with decomposition */
#ifdef DEBUG
        psv1 = clipped->pts;
        psf1 = clipped->eofl;
#endif
   ...
   DMESS "TRPfill: Clipped polygon(set) is multiple with %d vertices.\n"
        ...
        for (i = 0; i < maxdo; i++) {
            ...
            if (found) {  /* Trapezoid found, check its level */
                ...
                if (toluse) {  /* Round Y-coords to units of tolerance. */
                ...
                } else {  /* Tolerance not in use, keep trapezoid */
                    curtrap++;
                }
                if (curtrap - trap > numtz - 1) { /* Array of trapezoids is full, draw them. */
                    trpdraw(numtz, trap);
                    curtrap = trap;
#ifdef DEBUG
            ...
            } else {  /* Trapezoid not found, quit loop */
 ...
 */
{
    ...
    if (left_horiz) {
        if (right_horiz) { /* Both edges horizontal */
            cross = (le->loe->x > re->loe->x);
        } else {  /* Left edge horizontal and Right edge sloping */
            cross = (le->hix < le->loe->x);
        }
    } else {
        if (right_horiz) { /* Left edge sloping and right edge horizontal */
            cross = (re->hix > re->loe->x);
        } else { /* Both edges sloping, compare slopes to decide */
 ...
 */
{
    ...
    for (i = 0; i < n_et + 1; i++) {
        ...
        below = below->next;
        if (below == NULL) break;
        if (rem->loe == below->hiye) { /* Edge below has been found */
            trough = FALSE;
            break;
        }
    }

    if (trough) {  /* No edge below found, bottom must be a trough */
...
#endif
    } else {  /* Copy up edge below, except for top y vertex */
#ifdef DEBUG_FULL
 ...
 */
{
    ...
    if (TZEQUAL(le->hix, re->hix)) {  /* The two edges touch at the top */
        swap = edge_cross(le, re);
    } else swap = (le->hix > re->hix);

    if (swap) {  /* Edges need swapping, swap them */
        loeswap = le->loe;
        le->loe = re->loe;
        re->loe = loeswap;
        hixswap = le->hix;
...

{
    ...
    for (vertex = 0, curpt = pset->pts; vertex < pset->n; vertex++, curpt++) {    /* Find next vertex */
        if (polyset) {
            switch (*curfl) {
                case VIS:
                case INVIS:
        ...
        if (curpt->y > next->y) {  /* downward edge */
            top = curpt;
            bot = next;
        } else {  /* upward edge */
            top = next;
        ...
        for (j = 0; j < n_et; j++)   /* Loop over edge table */
        {
            ...
            if (e_poss->hiye->y <= top->y)  /* Sort on higher top-vertex */
            {
                ...
                if ((e_poss->hiye->y < top->y) || (e_poss->hiye >= top)) { /* Home found */
                    break;
                }
            }
            e_lastposs = e_poss;
 ...
 */
{
    ...
    DMESS "\net_validate start\n");
#endif
    for (j0 = 0; j0 < n_et; j0++) {  /* We may have to do several passes of the edge table */
        pass = FINALPASS;         /* Assume final pass, till contrary */
        a.sharp = FALSE;
        a.first = etab;
        a.second = etab->next;
        if (a.second != NULL) {  /* Non-empty edge table */
            for (j1 = 0; j1 < n_et + 1; j1++) {  /* Look for peak edge A */
                ...
                if (a.first->hiye == a.second->hiye) {  /* Peak edge found, call it A */
#ifdef DEBUG_FULL
                    ...
                    for (j2 = 0; j2 < n_et + 1; j2++) { /* Look for another peak edge at the same level */
                        b.previous = b.first; /* Next poss' peak edge */
                        b.first = b.second;
                        b.second = b.second->next;
                        if (b.second == NULL) break;
                        if (b.first->hiye == b.second->hiye) {   /* Peak found, call it B */
                            if (TZEQUAL(a.first->hiye->y, b.first->hiye->y)) {  /* Peak A and B at same level */
#ifdef DEBUG_FULL
                                DMESS "et_validate: Peaks A at %d & B at %d, level %f\n",
                                a.first->hiye-psv1, b.first->hiye-psv1, b.first->hiye->y);
#endif
                                if (a_dir_q) {
                                    if (a.first->hix < a.second->hix) {  /* 1st end of A left of 2nd */
                                        a.left = a.first;
                                        a.right = a.second;
                                    } else {  /* 2nd end of A left of 1st */
                                        a.left = a.second;
                                        a.right = a.first;
                                    }
                                ...
                                if (b.first->hix < b.second->hix) {  /* 1st end of B left of 2nd */
                                    b.left = b.first;
                                    b.right = b.second;
                                } else {  /* 2nd end of B left of 1st */
                                    b.left = b.second;
                                    b.right = b.first;
                                }
                       ...
                       b.first->hiye-psv1,b.left->hix,b.right->hix);
                                ...
                                if ((b.left->hix < a.right->hix + mindiff) &&
                                    (a.left->hix < b.right->hix + mindiff)) {  /* Peak edges A & B overlap or touch */
                                    if (a.right->hix < b.left->hix + mindiff) { /* A's right touches B's left */
                                        touching_peak(&a, &b, &pass);
                                        if (pass == NEWPASS) break;
                                    } else {
                                        if (b.right->hix < a.left->hix + mindiff) { /* B's right touches A's left */
                                            touching_peak(&b, &a, &pass);
                                            if (pass == NEWPASS) break;
                                        } else {   /* A and B overlap */
                                            if (a.left->hix < b.left->hix) { /* Overlap is from B-right to A-left */
                                                if (edge_swap(b.right, a.left)) {
                                                    a.sharp = TZEQUAL(a.left->hix, a.right->hix);
                                                    b.sharp = TZEQUAL(b.left->hix, b.right->hix);
                                                    pass = ANOTHERPASS;
                                                }
                                            } else { /* Overlap is from A-right to B-left */
                                ...
                                if ((a.sharp) && (a.left->loe == a.right->loe)) { /* Peak A degenerate, remove it */
                                    a.previous->next = a.second->next;
                                    pass = NEWPASS;
                                }
                                if ((b.sharp) && (b.left->loe == b.right->loe)) { /* Peak B degenerate, remove it */
 ...
 */
{
    ...
    if (horiz_edge_rm(&removed)) {  /* Pair of sloping edges found at top of edge table */
        if (removed > 0) {  /* Sort edge table to bring peaks together */
    ...
    if (side[0]->hix + mindiff < side[1]->hix) { /* Top 0 left of top 1 */
        side0_side1 = TRUE;
    } else {
        if (side[1]->hix + mindiff < side[0]->hix) {  /* Top 1 left of top 0 */
            side0_side1 = FALSE;
        } else {  /* Tops coincide */
    ...
    hipk.first = side[0];
    hipk.second = side[1];
    if (side0_side1) { /* Side 0 left of side 1 */
        hipk.left = side[0];
        hipk.right = side[1];
        hipk.left_slope = slope[0];
        hipk.right_slope = slope[1];
    } else { /* Side 1 left of side 0 */
        hipk.left = side[1];
        hipk.right = side[0];
...
static Logical horiz_edge_rm(Int *rm) {
    ...
    *rm = 0;  /* Initially no edges have been removed */
    ...
    for (j2 = 0; j2 < n_et; j2++) {
...
#endif
        if (top_y > pk2->hiye->y) { /* New edge not at top of edge table */
#ifdef DEBUG_FULL
            DMESS "horiz_edge_rm: non-top edge\n");
#endif
            if (nslope > 0) break;   /* If sloping edge found, quit loop */
            else { /* Top of edge table completely removed, down to new top */
                top_y = pk1->hiye->y;
#ifdef DEBUG_FULL
        ...
        if (pk1->hiye == pk2->hiye) {  /* Peak found */
            npeak++;               /* Count the peak */
            side = pk1;
            pkreset = FALSE;
            for (i = 0; i < 2; i++) { /* For each side of the peak */
...
#endif
                    bot = edge->loe;
                    if (VEQUAL(edge->hiye->y, edge->loe->y)) { /* Edge horizontal, remove it */
                        (void) edge_remove(edge);
                        (*rm)++;                        /* Count removed edge */
                        if (edge->loe == bot) {  /* Trough reached */
#ifdef DEBUG_FULL
                            DMESS "horiz_edge_rm: trough at vertex %d\n",bot-psv1);
#endif
                            if (pk1->loe == pk2->loe) {  /* Trough connects peak, remove peak completely */
#ifdef DEBUG_FULL
    ...
    if (npeak < 1) {  /* No peaks found, corrupt edge-table */
 ...
 */
{
    ...
    if (tz->base < hipk.left->hiye->y) {  /* Truncation is below peak */
        ...
        for (i = 0; i < n_et; i++) {
            ...
            /* Pass higher Y, */
            if (edge->hiye->y <= tz->base) { /* but not lower Y */
 ...
 */
{
    ...
    if (lp->sharp) {  /* Left Peak is a single point */
        if (lp->left->loe == rp->left->loe) { /* left sides share bottom end, swap sides of left peak */
            eswap = lp->left;
            lp->left = lp->right;
            lp->right = eswap;
        } else {
            if (rp->sharp) { /* Both peaks are a single points */
                if (lp->left->loe == rp->right->loe) {  /* outer sides share bottom end, swap sides to inner */
                    eswap = lp->left;
                    lp->left = lp->right;
                    lp->right = eswap;
                    eswap = rp->left;
    ...
    if (rp->sharp) { /* Right peak B a single point */
        if (lp->right->loe == rp->right->loe) { /* Both right sides share bottom end, swap sides of right peak */
            eswap = rp->left;
            rp->left = rp->right;
            rp->right = eswap;
        }
    }
    if (lp->right->loe == rp->left->loe) {  /* The peak edges need joining, join them */
        ...
        *pass = NEWPASS;
    } else { /* Swap two edges, if necessary */
...
static Logical trap_extract(Float ymin, Trapezoid *tz) {
...

    {
        ...
        for (i = 0, edge = etab; i < n_et; i++) {
            ...
            if (possible) {   /* Edge  possibly has top inside provisional trapezoid */
                if (edge->hiye->y > tz->base) {  /* Edge is high enough to be inside prov' trap' */
                    if ((tzleft < edge->hix) &&
                        (edge->hix < tzright)) { /* Such an edge found, raise base to it and quit loop.*/
                        tz->base = edge->hiye->y;
                        peak_change = TRUE;
                        ...
                        if (tzleft > tzright) {  /* The sides of the prov' trap' have crossed */
...

    {
        ...
        side = hpeak->left;
        slope = &(hpeak->left_slope);
        for (j = 0; j < 2; j++) { /* For each side of the provisional trapezoid */
            if (etab->next == NULL) break;
            for (i = 0, e2 = etab->next; i < n_et - 2; i++) {  /* For each of the other edges */
                e1 = e2;
                e2 = e1->next;
                if (e2 == NULL) break;
                if (e2->hiye->y < tz->base) break;

                if (e1->hiye == e2->hiye) { /* Implicit Peak edge found */
                    /* Find left and right */
                    if (e1->hix <= e2->hix) {
                        eleft = e1;
                        eright = e2;
                    ...
                    if (TZEQUAL(eleft->hix, tzend)) {
                        if (TZEQUAL(eright->hix, tzend)) {  /* side intersects peak */
                            cross = edge_cross(side, eleft) &&
                                    edge_cross(eright, side);
                        } else {  /* side intersects left-end of peak edge */
                            if (side == hpeak->right) {  /* peak edge from outside prov' trap' */
                                cross = edge_cross(side, eleft);
                            } else {  /* peak edge from inside prov' trap' */
                                cross = (tz->top_left < tz->top_right) ||
                                        (tz->top > side->hiye->y);
                            }  /* may be tangent */
                        }
                    } else {
                        if (TZEQUAL(eright->hix, tzend)) { /* Side intersects right end of peak edge */
                            if (side == hpeak->left) {  /* Peak edge from outside prov' trap' */
                    ...
                    if (cross) { /* Crossing found, raise base of prov' trap' to it */
                        tz->base = e1->hiye->y;
                        base_ends_off = TRUE;
                        base_cross = FALSE;
                        peak_change = TRUE;
...

    {
        ...
        side = hpeak->left;
        for (j = 0; j < 2; j++) { /* For each side of the provisional trapezoid? */
            for (i = 0, edge = etab; i < n_et + 1; i++) {  /* For each edge */
                edge = edge->next;
                ...
                if (edge->loe->y <= tz->top) { /* There is a common Y-range: determine intersection */
                    shie.x = side->hix;
                    shie.y = side->hiye->y;
                    ehie.x = edge->hix;
                    ehie.y = edge->hiye->y;
                    if (GRAcross(&shie, side->loe, &ehie, edge->loe,
                                 &collin, &crossing)) {
                        if (!collin) {  /* Intersection found, raise base to it */
                            if (crossing.y > tz->base) { /* Crossing above base of provisional trapezoid */
                                if (tz->top > crossing.y + mindiff) { /* Crossing below top of prov' trap' */
                                    tz->base = crossing.y;
                                    base_ends_off = TRUE;
    ...
    {
        if (base_ends_off) {  /* Calculate base ends */
            tz->base_left = tz->top_left +
                            hpeak->left_slope * (tz->top - tz->base);
            if (base_cross) { /* Base is an intersection, just one point */
                tz->base_right = tz->base_left;
            } else { /* Base has length */
        ...
        if ((rm_left && rm_right) &&
            (hpeak->left->loe == hpeak->right->loe)) {  /* The two sides of the trapezoid meet at base */
            etab->next = hpeak->second->next; /* Just, remove the peak */
        } else {  /* Truncate peak to base of trapezoid */
            peak_truncate(tz);
    ...
    /* Check the edge table for removal of trapezoid */
    {
        ...
        if (n_new > 0) { /* At least one edge still in edge table */
            if (!found) {  /* Trapezoid may have been removed without shortening the
               edge-table, but then it may not lengthened */
                if (n_new > n_old) { /* Removal has caused edge table to become longer! */
#ifdef DEBUG
                ...
                for (i = 0; i < 4 && i < n_new; i++) { /* Look for change in array indices of edges */
                ...
                if (!found) {  /* No change found, look for others */
                    for (i = 1; i < 4 && i < n_new; i++) { /* Look for a change in the edges */
