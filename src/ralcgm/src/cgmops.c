...
void CGMOps(FILE *stream, Code c, Long *pi, float *pr, char *str) {
    ...
    switch (major) {
        case 0x00:               /* Graphics Primitives  */
            switch (c) {
                ...
                case POLYGON:         /*  Polygon   */
                    if (!cont_list) {
                        ...
                        if (poly_mode == DECOMPOSE &&
                            PSdodecomp(num, pi, pr, NOSET)) { /* Trapezoid decomposition to be done */
                            PStrapfill(num, pi, pr, NOSET, FALSE);
                        } else {
                            PSOUT "gsave newpath  %%create path of polygon\n");
                            PSpoints(num, pi, pr, NOSET, " m ", " l ");
                            PSOUT "\n");
                            if (num > 0) { /* No more points, complete the fill */
                ...
                case POLYGONSET:         /*  Polygon Set  */
                    ...
                    if (poly_mode == DECOMPOSE &&
                        (cont_list || PSdodecomp(num, pi, pr, SET))) {  /* Do trapezoid decomposition */
            ...
            switch (c) {
                ...
                case ENDPIC:         /*  End  Picture    */
                    ...
                    if (ps_mode == EPSF)
                        ...
                        PSOUT "%s\n", "%%PageTrailer");
                        if (dev_ori == orient) {  /* Axes not rotated */
                            PSOUT "%s %d %d %d %d\n", "%%PageBoundingBox:",
                                  PSdev[PSdevice].x, PSdev[PSdevice].y,
                                  PSdev[PSdevice].x + x, PSdev[PSdevice].y + y);
                        } else {  /* Axes rotated, but PageBoundingBox NOT rotated */
                            PSOUT "%s %d %d %d %d\n", "%%PageBoundingBox:",
                                  PSdev[PSdevice].x, PSdev[PSdevice].y,
        ...
        case 0x31:  /* Metafile Descriptor Elements  */
            switch (c) {  /* NB: Nothing that writes into the Postscript file
...
static void PSbegpic(void) {
    ...
    if (dev_ori != orient) {
        l_marg = PSdev[PSdevice].y;
        r_marg = PSdev[PSdevice].Y;
        b_marg = PSdev[PSdevice].x;
        u_marg = PSdev[PSdevice].X;
    } else {
        l_marg = PSdev[PSdevice].x;
        r_marg = PSdev[PSdevice].X;
        b_marg = PSdev[PSdevice].y;
        u_marg = PSdev[PSdevice].Y;
    }
    ps_xrange = r_marg - l_marg;
    ps_yrange = u_marg - b_marg;

    /* RTP Work out scale in x and y and take minimum */
    if (ps_yrange / ps_xrange > vdc_yrange / vdc_xrange) {  /* Bounding box touches left and right of display */
        touching_sides = LANDSCAPE;
        psbb = ps_xrange;
        vdcbb = vdc_xrange;
    } else {  /* Bounding box touches top and bottom of display */
    ...
    /* Scale to VDC units */
    PSOUT "%f %f scale\n", (right - left) / vdc_xrange * scale1, (top - bot) / vdc_yrange * scale1);
    ...
    switch (int_style) {
        ...
        case PATTERN:     /* fill interior with predefined pattern  */
            ...
            if (PSpat_table[pat_ind].pathead[0] == INDPATTERN)
                for (i = 1; i < (PSpat_table[pat_ind].ny * PSpat_table[pat_ind].nx * 3); i += 3) {
...
static Enum PSarcref(PS_arcref *ref, double xa, double ya,
    ...
    xab = xb - xa;
    yab = yb - ya;
    absq = SQR(xab) + SQR(yab);
    xac = xc - xa;
    yac = yc - ya;
    acsq = SQR(xac) + SQR(yac);
...
static void PSpset(Long n, Long *pi, float *pr, Logical cont) {
    ...
    if (n > 2) {  /* No more points */

        if (PSfillatt.style != EMPTY) { /* Draw polygon set */
    ...
    if (n >= 0) {  /* No more points, free the space allocated */
        FREE(polset.eofl);
        FREE(polset.pts);
    }
...
static void PStrapfill(Long np, Long *pi, float *pr,
    ...
    if (np > 2) {  /* No more points */
        ...
        if ((clip.left < 0.0) || (clip.bot < 0.0)) {  /* Allow for negative coordinates */
            tzno = 40 / (3 * tzdec_all + 11);
        } else {  /* Can assume all coordinates are positive (TRPfill clips) */
    ...
    if (np >= 0) {  /* No more points, free the space allocated */
        FREE(polset.eofl);
        FREE(polset.pts);
    }
...
void PSTRPdraw(Int ntz, Trapezoid *tz) {
    ...
    no = tzno;
    prec = tzdec_right;
    for (i = 0, k = tzno + 1; i < ntz; i++, tz++) {  /* For each trapezoid */
        if (!--k) {
            PSOUT "\n");
            k = no;
        }
...
static void PSendpsfile(void) {
    ...
    if (ps_mode == EPSF) {
        ...
        if (dev_ori == orient) {  /* Axes not rotated */
            PSOUT "%s 0 0 %d %d\n", "%%BoundingBox:", x, y);
        } else { /* Axes rotated, but BoundingBox NOT rotated */
            PSOUT "%s 0 0 %d %d\n", "%%BoundingBox:", y, x);
        }
    } else
        PSOUT "%s %d\n", "%%Pages:", no_of_page);
