...
/* Ensure that colour table is able receive entries */
    if (colour_table == NULL) {  /* Colour table needs initialising */
        if (nc > INITLEN) {
            newlen = nc;
        } else {
            newlen = INITLEN;
        ...
        if (ncol_defined + nc > ncol_max) { /* Entries won't fit in vacant spaces */
            ncol_ignore = ncol_fixed;
            ncshort = 0;
            for (i = first; i < first + nc; i++) { /* Count new entries that can not replace previous */
                if (COLRtab(i) == NULL) ncshort++;
            }
            ncol_ignore = 0L;
            if (ncol_defined + ncshort > ncol_max) { /* Longer colour table needed - length multiple of INITLEN */
                ...
                ncol_max = newlen;
            } else {  /* We replace last preceeding entry of same index */
                replace = TRUE;
            }
...
/* Set new entries */
    if (replace) { /* We replace last entry of same index with new */
        ncol_ignore = ncol_fixed;
        for (i = first; i < first + nc; i++) {
            ...
            if (col != NULL) {
                ...
                col->green = (Posint) *pi++;
                col->blue = (Posint) *pi++;
            } else {  /* No changeable Entry of Current Index Found */
        ...
        ncol_ignore = 0L;
    } else {  /* We add all new entries to end */
    ...
    if (cgmstate == MF_DEFAULTS) {  /* We are in Metafile Defaults Replacement: Fix the entries */
