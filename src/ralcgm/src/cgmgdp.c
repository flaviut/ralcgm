...
/*  Produce list of points */
    ...
    BEGINLIST(pt);
    if (close == PIE) {   /*   If close is PIE first point is centre */
        STOREPOINT(pt, (*centre));
    }
    for (j = 0, ang = theta1; j < *np; j++, ang += dtheta) {
...

{
    ...
    DMESS " gdptype = %d",gdp);
#endif
    if (gdp != GKSCIRCLE) {  /* GDP with start/on/end points */
        start = pt + 3;
        on = pt + 4;
        end = pt + 5;      /* Name points */
        gdp3pts = TRUE;                           /* 3pt GDP */
    } else {  /* GDP circle with centre/on points */
        cen = pt + 4;
        on = pt + 3;                 /* Name points */
    ...
    if (gdp3pts && (FABS(XPROD(start, on, on, end)) < prodmin)) {  /* Points are Collinear: return as line */
        *pt = *start;
        *(pt + 1) = *on;
        *(pt + 2) = *end;
        *type = LINE;
    ...
    if (!circular) {  /* Elliptical Curve needed */
        det = (cdv1.x * cdv2.y - cdv1.y * cdv2.x);
        if (FABS(det) < prodmin) {  /* Tranformation has no inverse: return as line */
    ...
    if (gdp3pts) {  /* GDP Arc, Chord or Pie */
        if (circular) {  /* Part of Circle */
            *pt = *start;
            *(pt + 1) = *on;
            *(pt + 2) = *end;
            if (*close == NOCLOSE) *type = ARC3PT;
