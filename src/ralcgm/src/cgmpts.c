...

{
    ...
    if (np < 1) {  /* Number of points less than 1 */
        return;
    }

    limit = pt + (int) np;  /* Set limit to range of points */

    if (fl == NULL) {   /* Simple Points List (not polygon set) */
...
*/
{
    ...
    end1 = pt;
    end2 = pt + 1;         /* 1st edge between 1st two points */
    ...
    if ((clx1 == 0) && (cly1 == 0)) {  /* 1st end inside: initialise edge status as INSIDE */
        edgestatus = INSIDE;
    } else {  /* 1st end outside: initialise edge status as BOTHOUT */
        edgestatus = BOTHOUT;
    }
    ...
    for (i = 1; i < np; i++, end1++, end2++) {
        ...
        if ((clx2 == 0) && (cly2 == 0)) { /* 2nd end inside clipping rectangle */
        ...
        } else { /* 2nd end outside clipping rectangle */
            switch (edgestatus) /* status of last edge or as initialised */
            {
                case INCOMING:
                case INSIDE:
        ...
        switch (edgestatus)   /* Act according to edgestatus */
        {
            ...
            case INCOMING:  /* 1st end Outside and 2nd end Inside */
                ...
                ptclip = end1;
                eclp = end1;
                eoth = end2;  /* Set First end to be clipped. */
                clpx = &clx1;
                clpy = &cly1;
            ...
            case OUTGOING:  /* 1st end Inside and 2nd end Outside */
                eclp = end2;
                eoth = end1;  /* Set Second end to be clipped */
                clpx = &clx2;
                clpy = &cly2;
            ...
            case BOTHOUT:  /* Both ends Outside */
                ...
                if (((clx1 != clx2) || (clx1 == 0)) &&
                    ((cly1 != cly2) || (cly1 == 0))) {  /* edge ends by different sides of rectangle */
                    if (CLOCK(end1, &ul, end2) ||
                        CLOCK(end1, &ur, end2) ||
                        CLOCK(end1, &ll, end2) ||
                        CLOCK(end1, &lr, end2)) {  /* edge does not pass rectangle clockwise */
                        if (CLOCK(end1, end2, &ul) ||
                            CLOCK(end1, end2, &ur) ||
                            CLOCK(end1, end2, &ll) ||
                            CLOCK(end1, end2, &lr)) {    /* edge crosses clipping rectangle */
        ...
        if ((nendcl > 0) && (nendcl < 3)) {
            for (j = 0; j < nendcl; j++) /* Backward and Forward clip as required */
            {
                ...
                if (j < nendcl - 1) {   /* prepare for 2nd run of loop */
            ...
            switch (edgestatus) {
                case OUTGOING:
                case BOTHOUT: /* Component complete */
        ...
        clx1 = clx2;
        cly1 = cly2; /* Set Clipcodes for next 1st end */
...
/* NOTE much of the code here is duplicated in PTSget */
{
    ...
    if (np > NSTATPTS) {   /* Static store is insufficient, use dynamic */
        ...
        pt1 = dynpts;
    } else {   /* The static store has sufficient space, use it */
        pt1 = statpts;
    }
...

{
    ...
    if (clipind == ON) {        /* The calling routine requests clipping */
        PTSclipget(&aclipind, &clip);
        if (aclipind == ON) {        /* The CGM requests clipping */
            if ((clip.right - clip.left > 2 * msize) &&
                (clip.top - clip.bot > 2 * msize)) {        /* Markers within inclip would not be clipped */
                inclip.left = clip.left + msize;
                inclip.right = clip.right - msize;
                inclip.bot = clip.bot + msize;
                inclip.top = clip.top - msize;
            } else {        /* All markers could be clipped */
                inclip.left = inclip.right = inclip.bot = inclip.top = 0.0;
            }
        } else {  /* The CGM requests no clipping: do not clip */
            clipind = OFF;
        }
    ...
    /* Draw Markers */
    for (i = 0, mkcen = pt; i < nmk; i++, mkcen++) {                          /* For each Marker */
        if (clipind == ON) { /* Determine if Marker is to be drawn and/or clipped */
            tpt = *mkcen;
            if ((CLIPCODE(tpt.x, clip.left, clip.right)) ||
                (CLIPCODE(tpt.y, clip.bot, clip.top))) {  /* Marker Centre is outside clipping rectangle */
                clipmark = NODRAW;  /* Do not draw it */
            } else {
                if ((CLIPCODE(tpt.x, inclip.left, inclip.right)) ||
                    (CLIPCODE(tpt.y, inclip.bot, inclip.top))) {  /* Marker may be partially clipped */
                    clipmark = CLIPDRAW;   /* Clip Marker */
                } else {  /* Marker entirely within clipping rectangle */
                    clipmark = DRAWALL;   /* Draw without clipping */
                }
            }
        }

        if (clipmark != NODRAW) {     /* If current Marker is being drawn */
            for (j = 0, mkdp = mkdef; j < nmkl; j++) {                       /* For each Marker Line */
                for (k = 0, mklp = mkline;
                     k < nlp; k++, mkdp++, mklp++) {                   /* For each Marker Line Point */
