 ...
 *
 * Description: Initial values for CGM structures
 *
 * Modification log:
 *
 *   2 Aug 91 RTP  Add SCCS id
 *   7 Aug 91 RTP  Use CGMVERSION value instead of defined VERSIONx
 *  25 Sep 91 RTP  Change default max colours to 255 (from 63)
 *   2 Apr 93 RTP  Add Version 3 elements
 *   9 Aug 94 KEVP Add Windows Output Driver Elements
 *  12 Aug 94 KEVP Add flag cgmfinished to allow ending by return rather than exit
 *  13 Feb 94 KEVP Make Windows display driver default for MiscroSoft Windows
 *   1 Mar 95 KEVP Add variable to indicate when necessary any overrun of opcode
 *                 by the input driver (CGMIchar does it to BEGPIC after FONTLIST)
 *  22 Mar 95 KEVP Add variable for quiet output option
 *  29 Mar 95 KEVP Modify abbreviation for Windows Driver
 *   1 Jun 95 KEVP Revised default values minimum real and vdc absolute values,
 *                 to be low enough for all encodings (including binary).
 */
...
/*  Flag to indicate type of Character coding */

  Logical cgminnative = FALSE;
...
static const struct cgmdrivers drivers[] = {

        "CHARACTER", 2, CHARACTER,
        "NATIVE", 1, NATIVE,
        "CLEARTEXT", 2, CLEAR_TEXT,
        "TEXT", 3, CLEAR_TEXT,
        "BINARY", 1, BINARY,
...
const struct text_defaults textdef = {
        ...
        32767.0,        /* max real */
        4,        /* digits */
        -32767, 0.0,  /* min VDC (int,  real) */
        32767, 1.0,  /* max VDC (int,  real) */
        4,        /* digits */
        127,        /* colour index*/
...
const struct defaults commondef = {
        ...
        0, 0.0, 0, 0.0,   /*  vdc extent - bottom left */
        32767, 1.0, 32767, 1.0,                 /* top right */
        0, 0.0, 0, 0.0,   /*  clip rectangle  - bottom left */
        32767, 1.0, 32767, 1.0,                      /* top right */
...
const struct attributes defatt = {
        ...
        OFF,         /* edge vis */
        1.0,         /* continuous horiz alignment */
        1.0,         /* continuous vertical alignment */
        1,          /* character set */
        1,          /* alt character set  */
        1.0,          /* character expansion */
        0.0,          /* character space */
        0, 0.0, 1, 1.0,  /* character up vector*/
        1, 1.0, 0, 0.0,  /* character base vector */
        327, 0.01,        /* character height  */
        0,         /* interior style */
        1,         /* hatch index */
        1,         /* pattern index */
        0, 0.0, 0, 0.0, /* fill Reference point */
        0, 0.0, 1, 1.0, /* pattern size - height */
        1, 1.0, 0, 0.0,                /* width */
...
/*   Communications area to pass data from interpreter to driver */

  long pint[ARRAY_MAX], *pimax = pint + ARRAY_MAX;
  float preal[ARRAY_MAX], *prmax = preal + ARRAY_MAX;
  char str[STRING_MAX], *strmax = str + STRING_MAX;
#else

/*  to be used for Dynamic allocation */
...
Enum cgmprof = NONE;    /*  Profile flag set to none  */
...
Logical cgmrandom = FALSE;
long cgmnext = 1, cgmpresent = 0, cgmabort = 0, cgmoverrun = 0;
